...
with tab_nuevo:
    ...
    ent_nombre = pre1.selectbox("🏦 Entidad", sorted(dict_entidades.keys()), key="inv_ent")
    ...
    tipo_mov = pre3.selectbox("↕️ Tipo de operación", TIPOS_INVERSION, key="inv_tipo")
    ...
    with st.form(f"form_inversion_{st.session_state.inv_tick}", clear_on_submit=False):
        ...
        estrategia = c2.selectbox("🏷️ Estrategia", categorias_ord, index=idx_ninguna)
        ...
        imp = c10.number_input("Impuestos / Retención (€)", min_value=0.0, step=0.5, format="%.2f")

        obs = st.text_input("📝 Observaciones", placeholder="Ej: Aportación mensual indexado...")

        st.divider()

        btn_col1, btn_col2 = st.columns([1, 1])
        btn_calcular = btn_col1.form_submit_button("🔄 Calcular operación", width='stretch')
    ...
    if btn_guardar and st.session_state.inv_preview is not None:
        ...
        if p["activo_nombre"] == "— sin activos —":
            ...
            try:
                ...
                # Contramovimiento de tesorería con origen_id automático
                if isin_cuenta:
                    tipo_contra  = "Gasto" if p["tipo"] == "Compra" else "Ingreso"
                    # origen automático según tipo de operación
                    origen_auto  = "Compra de activo" if p["tipo"] == "Compra" else "Venta de activo"
                    origen_id    = dict_origenes.get(origen_auto)
                    ...
                    sql_tesoreria = text("""
                        INSERT INTO transacciones (
                            ...
                            unidades, precio_unitario,
                            importe_neto, comision, canon, impuestos, importe_total,
                            isin_origen_destino, estrategia_id, observaciones, origen_id
                        ) VALUES (
                            :fecha, :eid, :isin,
                            :tipo_id, 'TESORERIA',
                            0, 0,
                            :total, 0, 0, 0, :total,
                            :isin_activo, :estrategia_id, :obs, :origen_id
                        )
                    """)
                    params_tesoreria = {
                        "fecha":         p["fecha"],
    ...
    if df_edit_source.empty:
        ...
        if len(selection.selection.rows) > 0:
            ...
            with st.form(f"form_edicion_inv_{mov_id}"):
                ...
                btn_col1, btn_col2 = st.columns([1, 1])
                btn_calcular_e = btn_col1.form_submit_button("🔄 Calcular operación", width='stretch')
            ...
            if st.session_state.inv_edit_preview is not None:
                ...
                _label_cuenta = "💳 **Cargo en:** " if p["tipo"] == "Compra" else "💳 **Abono en:** "
                _cuenta_info = (
                ...
                st.caption("✅ Revisa la operación. Si es correcta pulsa **Actualizar Operación**.")
            ...
            with st.expander("⚠️ Zona peligrosa: eliminar operación"):
                ...
                if st.button("🗑️ Eliminar", type="primary", disabled=not confirmar):
                    try:
                        ...
                        msg_ok = "Operación de inversión y contramovimiento de tesorería eliminados." if tiene_contra else "Operación eliminada."
                        st.success(msg_ok)
                        st.session_state.inv_edit_preview     = None
                        st.session_state.inv_edit_preview_for = None
                        time.sleep(1)
